...
public class Window {
    ...
    public Rectangle scrollTo(Point point) {
        Object result = driver.executeScript(String.format("window.scrollTo(%d, %d);", point.x, point.y) + jsViewportTupleSnippet);
        return convertJsTupleToRectangle(result);
    }

    private final String jsViewportTupleSnippet = "return [window.scrollX, window.scrollY, window.innerWidth, window.innerHeight];";
